...
pacman::p_load(tidyverse, patchwork, ggrepel, ggmisc, ggiraph, DT, 
  gifski, plotly, shiny, gganimate, ggpp, ggtext, ggdist, ggridges, colorspace, geomtextpath, nord, nortest, seriation, dendextend, heatmaply, ggthemes, hrbrthemes, ggstatsplot) 
```
...
We first perform feature engineering to create new features that can be used in our charts and also analyse the distributions of key variables in the process.
...
### Univariate Distribution of Key Numerical Variables
...
From the histogram panel, we note that Driver Age and Driving Experience are approximately symmetric (skewness < 0.3, IQR ratio < 0.1), while Vehicle Value and Total Premium are heavily right-skewed (skewness > 2.0). **This is typical of insurance portfolios where a small number of high-value policies pull the mean above the median**. (The t-tests are based on whether mean and median are equal, and the test rejections are driven by sample size sensitivity rather than practically meaningful asymmetry).

The following table summarises the key insights from deeper analyses on polichholder characteristics and claims behaviour, while the tabs provide the charts along with the relevant explanations.
...
In this section, the portfolio is segmented into three groups based on loss ratio to assess how changes within each segment may affect overall profitability:

* Profitable (LR < 70%) – Premiums comfortably exceed claims, providing a healthy buffer against adverse shocks.
* At-Risk (70% ≤ LR < 100%) – Still profitable, but operating on thin margins and vulnerable to worsening claims experience.
* Loss-Making (LR ≥ 100%) – Claims exceed premiums, resulting in negative profitability and requiring immediate intervention.
